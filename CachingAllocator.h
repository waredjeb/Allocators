...
public:
    ...
    void* allocate(std::size_t size) {
        ...
        if (binIndex <= MAX_BINS) {
            if (!cached_blocks[binIndex].empty()) {
                const MemoryBlock cachedMemoryBlock = std::move(cached_blocks[binIndex].back());
    ...
    void deallocate(void* ptr) {
        for (size_t i = 0; i < live_blocks.size(); ++i) {
            ...
            if (it != bin.end()) {
                if (debug_)
                    printf("Deallocating block %p of size %lld to bin %lld\n", ptr, static_cast<long long>(it->size), static_cast<long long>(i));
...
private:
    ...
    std::size_t findBin(std::size_t size) {
        ...
        return static_cast<std::size_t>(std::ceil(std::log(size) / std::log(GROWTH)));
    }
};
